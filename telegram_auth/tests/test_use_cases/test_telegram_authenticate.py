...
# Проверяет, что функция не вызывает исключения, если пользователь существует
...
# Проверяет, что функция вызывает исключение, если пользователь не найден
@pytest.mark.django_db
def test_ensure_user_exists_error():
    with pytest.raises(UserNotFoundError):
        ensure_user_exists(999)

# Проверяет, что upsert_user создает нового пользователя, если его не существует
@pytest.mark.django_db
...
# Проверяет, что upsert_user обновляет данные существующего пользователя
@pytest.mark.django_db
...
# Проверяет, что full_name пользователя корректно собирается из имени и фамилии
...
# Проверяет, что full_name пользователя корректно работает без фамилии
